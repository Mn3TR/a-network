"""Generate Colab notebook with proper nbformat"""
...
cells.append(nbf.v4.new_code_cell(
    ...
    "LEARNING_RATE = 1e-4\n"
    "GRAD_ACCUM = 4\n"
    "BATCH_SIZE = 8            # 并行序列数（越大越快，消耗显存更多）\n"
    "\n"
    "# 数据集（支持 '数据集名' 或 '数据集名,config名,split=名称'）\n"
...
cells.append(nbf.v4.new_code_cell(
    ...
    "        lr = 1e-6 + (LEARNING_RATE - 1e-6) * (1 + math.cos(math.pi * p)) * 0.5\n"
    ...
    "\n"
    "    for step, (inputs, targets) in enumerate(pack_batch(tokens, B)):\n"
...
# Colab 要求 id 在 metadata 里，不在 cell 顶层
for idx, c in enumerate(nb.cells):
    cid = c.pop('id', None)  # 移除顶层 id
    if cid:
        c['metadata']['id'] = cid
    elif 'id' not in c['metadata']:
        c['metadata']['id'] = f'cell_{idx}'
